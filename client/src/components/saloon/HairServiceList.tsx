...
export default function HairServiceList() {
        ...
        const fetchSubCategories = async () => {
            try {
                ...
                if (result.success && result.data) {
                    // Filter for "Hair Services" (ID: 69bcee74b213eff695c98a92)
                    const filtered = result.data.filter((sub: any) => {
                        const cid = typeof sub.categoryId === 'object' ? sub.categoryId?._id : sub.categoryId;
                        return cid === '69bcee74b213eff695c98a92';
                    });
                    setSubCategories(filtered);
                }
            } catch (err) {
                console.error("Hair Sub-Categories fetch failed", err);
            } finally {
                setLoading(false);
            }
        };
    ...
    const displayServices = subCategories.map((sub, i) => ({
        ...
        image: sub.cover_image ? `${process.env.NEXT_PUBLIC_API_BASE_URL}${sub.cover_image}` : "/saloon/assets/images/hair service/hair cut.png",
        category: "Hair"
    }));
        ...
        <section className="py-[70px] px-5 sm:px-14 max-w-[1280px] mx-auto min-h-[400px]">
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-20 opacity-30">
                    ...
                    <p className="text-[10px] uppercase tracking-[3px] italic">Curating specialized treatments...</p>
                </div>
                ...
                <ScrollReveal className="flex flex-col items-center justify-center py-24 border border-white/[0.03] bg-white/[0.01] rounded-lg">
                    ...
                    <p className="text-salon-gray font-light tracking-[2px] uppercase text-[11px] text-center max-w-md px-6">
                        No hair care rituals are currently published. Please consult our artisans for a bespoke hair experience or check back soon.
                    </p>
                </ScrollReveal>
            ) : (
